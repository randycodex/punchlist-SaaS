...
export default function ZoningWorksheetSection({ section }: { section: ZoningReportSection }) {
  return (
    <section id={section.key} className="overflow-hidden rounded-lg border border-black/10 bg-white/80 dark:border-white/10 dark:bg-white/[0.04]">
      ...
      <div className="hidden grid-cols-[1.1fr_1.1fr_1fr_9rem_1.2fr] gap-3 px-4 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-gray-400 md:grid">
        <div>Field</div>
        <div>Value</div>
        <div>Source</div>
        <div>Status</div>
      ...
      {section.items.map((item) => (
        <ZoningWorksheetRow key={item.id} item={item} />
      ))}
    </section>
  );
}
