...
export async function GET(request: Request) {
  ...
  const params = new URLSearchParams({
    f: 'geojson',
    where: '1=1',
    outFields: 'BBL,LandUse,Lot,Address,BldgClass',
    returnGeometry: 'true',
  ...
  try {
    const response = await fetch(`${mapPlutoQueryUrl}?${params.toString()}`, {
      headers: { accept: 'application/geo+json, application/json' },
      cache: 'no-store',
    });

    if (!response.ok) {
      return NextResponse.json({ error: `MapPLUTO request failed: ${response.status}` }, { status: 502 });
    }

    const payload = await response.json();
    return NextResponse.json(payload);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unable to fetch MapPLUTO land-use data.';
